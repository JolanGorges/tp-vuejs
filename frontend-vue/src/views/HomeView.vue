<template>
  <div>
    ...
    <div class="row">
      <div class="col-4 ps-lg-5">
        <div v-if="bills" class="card p-2 mb-4">
          <div class="d-flex align-items-center">
            <h1 class="ps-2">{{ billsFromCurrentYear() }}</h1>
            <h5 class="ps-2">Factures en {{ year }}</h5>
          </div>
          <div class="d-flex justify-content-center">
            <DoughnutChart
              :totalBilled="totalBilled(year)"
              :totalCollected="totalCollected(year)"
              :totalOverdue="totalOverdue(year)"
            />
          </div>
          <h6 class="text-end">{{ totalBilled(year) }}€</h6>
          <Legend
            text="Total Encaissé TTC"
            color="text-primary"
            :percentage="totalCollectedPercentage(year)"
            :percentageVariation="totalBilledPercentageVariation(year)"
          />
          <Legend
            text="Total Facturé TTC"
            color="text-warning"
            :percentage="totalBilledPercentage(year)"
            :percentageVariation="totalCollectedPercentageVariation(year)"
          />
          <Legend
            text="Dû à plus d'un mois TTC"
            color="text-danger"
            :percentage="totalOverduePercentage(year)"
            :percentageVariation="totalOverduePercentageVariation(year)"
          />
        </div>
      </div>
      <div class="col-8 ps-lg-5">
        <div v-if="bills" class="card p-2 mb-4">
        ...
        <div v-if="clients" class="card p-2">
